...
__global__ void uint8_cross_block_echo_kernel(uint64_t* metrics, uint8_t* client_buf, uint8_t* server_buf,
...
int main(int argc, char** argv) {
    ...
    cudaError_t err = cudaLaunchCooperativeKernel((void*)uint8_cross_block_echo_kernel, gridDim, blockDim, kernelArgs);
    if (err != cudaSuccess) {
    ...
    std::filesystem::create_directories("data/" + deviceName + "-uint8");
    std::string name = "data/" + deviceName + "-uint8/metrics_" + std::to_string(msg_size) + "B_" + std::to_string(num_pairs) + "P.json";
