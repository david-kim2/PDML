...
__global__ void uint64_cross_block_echo_kernel(
    uint64_t* metrics, // Place to store output metrics, shape [n_runs][num_pairs][8]
    uint64_t* client_buf, // Global memory buffer for client-to-server messages
    uint64_t* server_buf, // Global memory buffer for server-to-client messages
    ...
    uint64_t* client_offset = client_buf + tid * msg_size_word;
    uint64_t* server_offset = server_buf + tid * msg_size_word;
