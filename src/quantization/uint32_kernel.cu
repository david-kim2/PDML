...
__global__ void uint32_cross_block_echo_kernel(
    uint64_t* metrics, // Place to store output metrics, shape [n_runs][num_pairs][8]
    uint32_t* client_buf, // Global memory buffer for client-to-server messages
    uint32_t* server_buf, // Global memory buffer for server-to-client messages
    ...
    uint32_t* client_offset = client_buf + tid * msg_size_word;
    uint32_t* server_offset = server_buf + tid * msg_size_word;
