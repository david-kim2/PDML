...
__global__ void uint16_cross_block_echo_kernel(uint64_t* metrics, uint16_t* client_buf, uint16_t* server_buf,
...
int main(int argc, char** argv) {
    ...
    size_t msg_size_word   = (msg_size_thread + 1) / 2;
    size_t buf_bytes       = num_pairs * msg_size_word * sizeof(uint16_t);
    ...
    cudaError_t err = cudaLaunchCooperativeKernel((void*)uint16_cross_block_echo_kernel, gridDim, blockDim, kernelArgs);
    if (err != cudaSuccess) {
    ...
    std::filesystem::create_directories("data/" + deviceName + "-uint16");
    std::string name = "data/" + deviceName + "-uint16/metrics_" + std::to_string(msg_size) + "B_" + std::to_string(num_pairs) + "P.json";
