...
__global__ void uint32_cross_block_echo_kernel(uint64_t* metrics, uint32_t* client_buf, uint32_t* server_buf,
...
int main(int argc, char** argv) {
    ...
    size_t buf_bytes       = num_pairs * msg_size_word * sizeof(uint32_t);
    ...
    cudaError_t err = cudaLaunchCooperativeKernel((void*)uint32_cross_block_echo_kernel, gridDim, blockDim, kernelArgs);
    if (err != cudaSuccess) {
    ...
    std::filesystem::create_directories("data/" + deviceName + "-uint32");
    std::string name = "data/" + deviceName + "-uint32/metrics_" + std::to_string(msg_size) + "B_" + std::to_string(num_pairs) + "P.json";
