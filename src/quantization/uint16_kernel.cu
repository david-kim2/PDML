...
__global__ void uint16_cross_block_echo_kernel(
    uint64_t* metrics, // Place to store output metrics, shape [n_runs][num_pairs][8]
    uint16_t* client_buf, // Global memory buffer for client-to-server messages
    uint16_t* server_buf, // Global memory buffer for server-to-client messages
    ...
    // Global memory layout, size = num_pairs * msg_size_word + num_pairs
    uint16_t* client_offset = client_buf + tid * msg_size_word;
    uint16_t* server_offset = server_buf + tid * msg_size_word;
    ...
    for (int run = 0; run < n_runs; run++) {
        ...
        if (bid == 0 && tid < num_pairs) { // CLIENT
            ...
            for (size_t i = 0; i < msg_size_word; i++)
                client_offset[i] = 0;
            finished_c2s[tid] = 0;
            __threadfence();
