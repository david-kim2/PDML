...
def plot_device_metrics(device_name, output_data, selected_pairs, selected_quantizations, args):
    ...
    quantizations                = np.array([entry["quantization"] for entry in output_data])
    ...
    iterable = [(p, q) for p in selected_pairs for q in selected_quantizations]
    for (pairs, quantization) in iterable:
        mask = (num_pairs == pairs) & (quantizations == quantization)
        ...
        color_idx = iterable.index((pairs, quantization))
        color = cm.viridis(color_idx / max(len(iterable) - 1, 1))

        category_label = device_name + f"-{quantization} (P={pairs})"
    ...
    for quantization in selected_quantizations:
        fabric_latencies_client = np.array([entry["metrics"]["fabric_latency_client_avg"] for entry in output_data \
                                    ...
                                    and entry["quantization"] == quantization])
    ...
    fig.legend(handles, labels, loc='upper right', bbox_to_anchor=(0.81, 0.875), fontsize=12)
    fig.text(0.69, 0.72, 'Solid: Client\nDashed: Server', fontsize=12, bbox=dict(boxstyle='round', facecolor='white', alpha=0.3))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pairs", type=int, nargs='+', default=[1024],
                        help="List of num_pairs to include in the graphs (e.g. --pairs 1 2 4). If omitted, include only 1024.")
    parser.add_argument("--quantizations", type=str, nargs='+', default=["uint8", "uint16", "uint32", "uint64"],
                        help="List of quantizations to include in the graphs (e.g. --quantizations uint8 uint16). If omitted, include all.")
    ...
    for device in devices:
        device_path_uint8  = os.path.join(args.data_dir, device) + "-uint8"
        json_files_uint8   = [f for f in os.listdir(device_path_uint8) if f.endswith('.json')]
        device_path_uint16 = os.path.join(args.data_dir, device) + "-uint16"
        json_files_uint16  = [f for f in os.listdir(device_path_uint16) if f.endswith('.json')]
        device_path_uint32 = os.path.join(args.data_dir, device) + "-uint32"
        json_files_uint32  = [f for f in os.listdir(device_path_uint32) if f.endswith('.json')]
        device_path_uint64 = os.path.join(args.data_dir, device) + "-uint64"
        json_files_uint64  = [f for f in os.listdir(device_path_uint64) if f.endswith('.json')]

        json_files = []
        json_files.extend([(f, device_path_uint8, "uint8")  for f in json_files_uint8])
        json_files.extend([(f, device_path_uint16, "uint16") for f in json_files_uint16])
        json_files.extend([(f, device_path_uint32, "uint32") for f in json_files_uint32])
        json_files.extend([(f, device_path_uint64, "uint64") for f in json_files_uint64])

        output_json_path = os.path.join(args.data_dir, f"{device}_metrics.json")
        output_data      = []

        for json_file, device_path, quantization in json_files:
            json_path = os.path.join(device_path, json_file)
            msg_size, num_pairs, num_runs, metrics = compute_metrics(json_path)
            output_data.append({
                'quantization': quantization, 'msg_size': msg_size, 'num_pairs': num_pairs,
                'num_runs': num_runs, 'metrics': metrics
            })

        output_data = sorted(output_data, key=lambda i: (i['msg_size'], i['num_pairs'], i['quantization']))
        with open(output_json_path, 'w') as f:
            json.dump(output_data, f, indent=4)

        plot_device_metrics(hwd_alias.get(device, device), output_data, args.pairs, args.quantizations, args)
