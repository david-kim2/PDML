...
def compute_max_args(min_mem, max_mem, fix_num_pairs):
    ...
    for num_pairs in possible_pairs:
        for msg_size in possible_msg_sizes:
            if msg_size >= min_mem and msg_size <= max_mem and msg_size % num_pairs == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-mem", type=int, default=0, help="Minimum msg size in KiB")
    parser.add_argument("--max-mem", type=int, required=True, help="Maximum msg size in KiB")
