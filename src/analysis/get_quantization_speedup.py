...
if __name__ == "__main__":
    ...
    area_path   = str(os.path.join("..", "quantization", "data"))
    ...
    with open(json_path, 'r') as f:
        ...
        unique_quantizations = ["uint8", "uint16", "uint32", "uint64"]
        data_dict = {"device": args.device, "speedups": [], "speedup_stats": []}

        for quant1, quant2 in itertools.combinations(unique_quantizations, 2):
            throughputs1 = [(m["msg_size"], m["metrics"]["round_trip_throughput_avg"]) for m in device_metrics if m["quantization"] == quant1]
            throughputs2 = [(m["msg_size"], m["metrics"]["round_trip_throughput_avg"]) for m in device_metrics if m["quantization"] == quant2]
            ...
            throughputs1_filtered = [(msg_size, thr) for msg_size, thr in throughputs1 if msg_size in common_msg_sizes]
            throughputs1_filtered = sorted(throughputs1_filtered, key=lambda x: x[0])
            throughputs2_filtered = [(msg_size, thr) for msg_size, thr in throughputs2 if msg_size in common_msg_sizes]
            throughputs2_filtered = sorted(throughputs2_filtered, key=lambda x: x[0])

            assert len(throughputs1_filtered) == len(throughputs2_filtered), "Mismatched number of common message sizes."
            ...
            data_dict["speedups"].append({ "quant_pair": [quant1, quant2], "speedup_values": speedups})
            data_dict["speedup_stats"].append({ "quant_pair": [quant1, quant2], "avg_speedup": avg_speedup, "std_speedup": std_speedup})
            print(f"Quantization Speedup from {quant1}->{quant2} on {args.device}: Avg={avg_speedup:.4f}, Std={std_speedup:.4f}")
    ...
    def compact_pairs_lists(json_str: str) -> str:
        ...
        return re.sub(r'"quant_pair"\s*:\s*\[\s*([\s\S]*?)\s*\]', repl, json_str)

    os.makedirs('data/quantization_speedup', exist_ok=True)
    output_path = f'data/quantization_speedup/{args.device}.json'
