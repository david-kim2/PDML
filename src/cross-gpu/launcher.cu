...
int main(int argc, char **argv)
{
    ...
    {
        if (my_pe == 0)
            std::cerr << "Error: Need 2 PEs for cross-GPU communication (got " << num_pes << ")" << std::endl;
        nvshmem_finalize();
        return -1;
    }
    ...
    if (my_pe == 0)
    {
        ...
        std::cout << std::endl;
        std::cout << "Launching cross-GPU echo kernel..." << std::endl;
    }   
    ...
    if (argc == 4)
    {
        // if we have arguments use them instead
    ...
    if (my_pe == 0)
        std::cout << "Launching cross-GPU echo kernel with message size " << msg_size << " bytes, " << num_pairs << " pairs, " << n_runs << " runs..." << std::endl;

    cross_gpu_echo_kernel<<<gridDim, blockDim>>>(
        d_metrics, 
        d_nvshmem_buffer,
        msg_size,
        num_pairs, 
    ...
    if (my_pe == 0)
    {
        std::cout << "Cross-GPU echo kernel completed." << std::endl;
        std::cout << std::endl;
    }
    ...
    if (my_pe == 0)
    {
        ...
        json_output["communication_type"] = "cross_gpu_nvshmem";
        ...
        std::string name = "data/" + deviceName + "/crossgpu_metrics_" + std::to_string(msg_size) + "B_" + std::to_string(num_pairs) + "P.json";
